...
class TestCancelPolicy(unittest.TestCase):
    ...
    def test_forceful_cancel_policy(self):
        self.assertEquals("Active", self.policy.status)
        pa = PolicyAccounting(self.policy.id)
        cancel_msg = "Test message to cancel policy"
        pa.cancel_policy(message=cancel_msg)
        self.assertEquals(self.policy.status, "Canceled")
        self.assertEquals(self.policy.cancel_message, cancel_msg)
